...
static const uint8_t security_read_data[] = {
    0xDE, 0xAD, 0xBE, 0xEF, 0x12, 0x34, 0x56, 0x78
};
static const uint8_t security_short_data[] = {
    0x10, 0x32, 0x54, 0x76
};

static int security_read_fn(const void *ctx_blob, uint32_t off, uint32_t len, uint8_t *dst) {
    (void)ctx_blob;
    if (!dst || off >= sizeof(security_read_data)) {
        return 0;
    }
    uint32_t avail = (uint32_t)sizeof(security_read_data) - off;
    uint32_t to_copy = len < avail ? len : avail;
    memcpy(dst, security_read_data + off, to_copy);
    return (int)to_copy;
}

static int security_short_read_fn(const void *ctx_blob, uint32_t off, uint32_t len, uint8_t *dst) {
    (void)ctx_blob;
    if (!dst || off >= sizeof(security_short_data)) {
        return 0;
    }
    uint32_t avail = (uint32_t)sizeof(security_short_data) - off;
    uint32_t to_copy = len < avail ? len : avail;
    memcpy(dst, security_short_data + off, to_copy);
    return (int)to_copy;
}
...
/*
 * Test 12b: Read context data via read_fn
 */
TEST(read_context_data_read_fn) {
    const char *js_code =
        "function mbpf_prog(ctx) {\n"
        "    if (ctx.data_len !== 8) return -1;\n"
        ...
        "    var n = ctx.readBytes(4, 4, buf);\n"
        "    if (n !== 4) return -3;\n"
        "    if (buf[0] !== 0x12) return -4;\n"
        "    if (buf[3] !== 0x78) return -5;\n"
        "    return 0;\n"
        "}\n";
    ...
    mbpf_ctx_security_v1_t ctx = {
        ...
        .action = 0,
        .flags = 0,
        .data_len = sizeof(security_read_data),
        .data = NULL,
        .read_fn = security_read_fn
    };
...
/*
 * Test 12c: Short read via read_fn updates data_len and bounds checks
 */
TEST(read_context_short_read_fn) {
    const char *js_code =
        "function mbpf_prog(ctx) {\n"
        "    if (ctx.data_len !== 4) return -1;\n"
        "    if (ctx.readU16LE(0) !== 0x3210) return -2;\n"
        "    try {\n"
        "        ctx.readU8(4);\n"
        "        return -3;\n"
        "    } catch (e) {\n"
    ...
    mbpf_ctx_security_v1_t ctx = {
        ...
        .data_len = 8,
        .data = NULL,
        .read_fn = security_short_read_fn
    };
...
int main(int argc, char *argv[]) {
    ...
    RUN_TEST(read_context_data_read_fn);
    RUN_TEST(read_context_short_read_fn);
