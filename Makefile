...
all: $(LIB) $(TEST_BIN) $(TEST_PKG_HEADER) $(TEST_PARSE_FILE) $(TEST_SECTION_TABLE) $(TEST_MANIFEST) $(TEST_BYTECODE) $(TEST_CRC) $(TEST_SIGNING) $(TEST_RUNTIME) $(TEST_PROGRAM_LOAD) $(TEST_PROGRAM_LOAD_VALIDATION) $(TEST_PROGRAM_UNLOAD) $(TEST_ATTACH_DETACH) $(TEST_ATTACH_VALIDATION) $(TEST_INSTANCE) $(TEST_RUN_BASIC) $(TEST_CONTEXT_OBJECT) $(TEST_CONTEXT_READ_METHODS) $(TEST_CONTEXT_NET_RX_V1) $(TEST_HOOK_TRACEPOINT) $(TEST_HOOK_TIMER) $(TEST_HOOK_NET_RX) $(TEST_HOOK_NET_TX) $(TEST_HOOK_SECURITY) $(TEST_HOOK_CUSTOM) $(TEST_ENTRY_POINT) $(TEST_MBPF_INIT) $(TEST_MBPF_FINI) $(TEST_RETURN_VALUE) $(TEST_EXCEPTION_HANDLING) $(TEST_ARRAY_MAP) $(TEST_HASH_MAP) $(TEST_MAP_MAX_ENTRIES) $(TEST_MAP_TYPE_VALIDATION) $(TEST_LRU_HASH_MAP) $(TEST_PERCPU_MAP) $(TEST_RING_BUFFER) $(CREATE_MBPF) $(MQJS)
