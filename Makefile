...
# Source files
MBPF_SRCS = $(SRC_DIR)/mbpf_runtime.c $(SRC_DIR)/mbpf_package.c $(SRC_DIR)/mbpf_stdlib.c $(SRC_DIR)/ed25519.c $(SRC_DIR)/mbpf_typed_array.c $(SRC_DIR)/mbpf_manifest_gen.c
...
TEST_CONTEXT_OBJECT_REUSE = $(BUILD_DIR)/test_context_object_reuse
...
TEST_MANIFEST_GEN = $(BUILD_DIR)/test_manifest_gen
TEST_MBPF_ASSEMBLE = $(BUILD_DIR)/test_mbpf_assemble
TEST_TOOLCHAIN_SIGNING = $(BUILD_DIR)/test_toolchain_signing
TEST_OBSERVABILITY_COUNTERS = $(BUILD_DIR)/test_observability_counters
TEST_TRACE_LOGGING = $(BUILD_DIR)/test_trace_logging
TEST_DEBUG_SYMBOLS = $(BUILD_DIR)/test_debug_symbols
TEST_TARGET_WORD_SIZE = $(BUILD_DIR)/test_target_word_size
TEST_TARGET_ENDIANNESS = $(BUILD_DIR)/test_target_endianness
TEST_EXAMPLE_NET_RX_FILTER = $(BUILD_DIR)/test_example_net_rx_filter
TEST_U64_REPRESENTATION = $(BUILD_DIR)/test_u64_representation
TEST_ROM_STDLIB = $(BUILD_DIR)/test_rom_stdlib
TEST_NO_DYNAMIC_ALLOC = $(BUILD_DIR)/test_no_dynamic_alloc
TEST_LOCK_FREE_MAP_READS = $(BUILD_DIR)/test_lock_free_map_reads
TEST_ERROR_INVALID_PACKAGE = $(BUILD_DIR)/test_error_invalid_package
TEST_ERROR_HOOK_MISMATCH = $(BUILD_DIR)/test_error_hook_mismatch
TEST_ERROR_CAPABILITY_DENIED = $(BUILD_DIR)/test_error_capability_denied
TEST_UNIT_PACKAGE_PARSER = $(BUILD_DIR)/test_unit_package_parser
TEST_UNIT_MAP_OPERATIONS = $(BUILD_DIR)/test_unit_map_operations
...
TEST_JS_CODEGEN_SIZING = $(BUILD_DIR)/test_js_codegen_sizing
TEST_MAP_NAME_ESCAPING = $(BUILD_DIR)/test_map_name_escaping
TEST_RING_BUFFER_SYNC_BOUNDS = $(BUILD_DIR)/test_ring_buffer_sync_bounds
TEST_MAP_WRITE_CONCURRENCY = $(BUILD_DIR)/test_map_write_concurrency
...
$(BUILD_DIR)/test_context_object_reuse: $(TEST_DIR)/test_context_object_reuse.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)
...
$(BUILD_DIR)/test_manifest_gen: $(TEST_DIR)/test_manifest_gen.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_mbpf_assemble: $(TEST_DIR)/test_mbpf_assemble.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_toolchain_signing: $(TEST_DIR)/test_toolchain_signing.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_observability_counters: $(TEST_DIR)/test_observability_counters.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_trace_logging: $(TEST_DIR)/test_trace_logging.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_debug_symbols: $(TEST_DIR)/test_debug_symbols.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_target_word_size: $(TEST_DIR)/test_target_word_size.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_target_endianness: $(TEST_DIR)/test_target_endianness.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_example_net_rx_filter: $(TEST_DIR)/test_example_net_rx_filter.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_u64_representation: $(TEST_DIR)/test_u64_representation.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_rom_stdlib: $(TEST_DIR)/test_rom_stdlib.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_no_dynamic_alloc: $(TEST_DIR)/test_no_dynamic_alloc.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS) \
		-Wl,--wrap=malloc,--wrap=free,--wrap=realloc,--wrap=calloc

$(BUILD_DIR)/test_lock_free_map_reads: $(TEST_DIR)/test_lock_free_map_reads.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS) -lpthread

$(BUILD_DIR)/test_error_invalid_package: $(TEST_DIR)/test_error_invalid_package.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_error_hook_mismatch: $(TEST_DIR)/test_error_hook_mismatch.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_error_capability_denied: $(TEST_DIR)/test_error_capability_denied.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_unit_package_parser: $(TEST_DIR)/test_unit_package_parser.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_unit_map_operations: $(TEST_DIR)/test_unit_map_operations.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_unit_budget_enforcement: $(TEST_DIR)/test_unit_budget_enforcement.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_integration_load_run: $(TEST_DIR)/test_integration_load_run.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)
...
$(BUILD_DIR)/test_js_codegen_sizing: $(TEST_DIR)/test_js_codegen_sizing.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_map_name_escaping: $(TEST_DIR)/test_map_name_escaping.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_ring_buffer_sync_bounds: $(TEST_DIR)/test_ring_buffer_sync_bounds.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/test_map_write_concurrency: $(TEST_DIR)/test_map_write_concurrency.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS) -lpthread
...
$(BUILD_DIR)/mbpf_manifest_gen: $(TOOLS_DIR)/mbpf_manifest_gen.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/mbpf_assemble: $(TOOLS_DIR)/mbpf_assemble.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

$(BUILD_DIR)/mbpf_sign: $(TOOLS_DIR)/mbpf_sign.c $(LIB) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(BUILD_DIR) -lmbpf $(LDFLAGS)

# Run tests
test: $(MQJS) $(ASSEMBLE_TOOL) $(TEST_BIN) $(TEST_PKG_HEADER) $(TEST_SECTION_TABLE) $(TEST_MANIFEST) $(TEST_BYTECODE) $(TEST_CRC) $(TEST_SIGNING) $(TEST_RUNTIME) $(TEST_PROGRAM_LOAD) $(TEST_PROGRAM_LOAD_VALIDATION) $(TEST_PROGRAM_UNLOAD) $(TEST_ATTACH_DETACH) $(TEST_ATTACH_VALIDATION) $(TEST_INSTANCE) $(TEST_RUN_BASIC) $(TEST_CONTEXT_OBJECT) $(TEST_CONTEXT_READ_METHODS) $(TEST_CONTEXT_NET_RX_V1) $(TEST_HOOK_TRACEPOINT) $(TEST_HOOK_TIMER) $(TEST_HOOK_NET_RX) $(TEST_HOOK_NET_TX) $(TEST_HOOK_SECURITY) $(TEST_HOOK_CUSTOM) $(TEST_ENTRY_POINT) $(TEST_MBPF_INIT) $(TEST_MBPF_FINI) $(TEST_RETURN_VALUE) $(TEST_EXCEPTION_HANDLING) $(TEST_ARRAY_MAP) $(TEST_HASH_MAP) $(TEST_MAP_MAX_ENTRIES) $(TEST_MAP_TYPE_VALIDATION) $(TEST_LRU_HASH_MAP) $(TEST_PERCPU_MAP) $(TEST_RING_BUFFER) $(TEST_COUNTER_MAP) $(TEST_MAP_ITERATION) $(TEST_MAP_PERSISTENCE) $(TEST_MAP_CONCURRENCY) $(TEST_HELPER_API_VERSION) $(TEST_HELPER_LOG) $(TEST_HELPER_U64_LOAD_STORE) $(TEST_HELPER_NOW_NS) $(TEST_HELPER_EMIT) $(TEST_MAP_LOWLEVEL) $(TEST_BUDGET_MAX_STEPS) $(TEST_HEAP_SIZE) $(TEST_TARGET_WORD_SIZE) $(TEST_TARGET_ENDIANNESS) $(TEST_EXAMPLE_NET_RX_FILTER)
	./$(TEST_BIN)
	./$(TEST_PKG_HEADER)
	./$(TEST_SECTION_TABLE)
	./$(TEST_MANIFEST)
	...
	./$(TEST_HEAP_SIZE)
	./$(TEST_TARGET_WORD_SIZE)
	./$(TEST_TARGET_ENDIANNESS)
	./$(TEST_EXAMPLE_NET_RX_FILTER)
